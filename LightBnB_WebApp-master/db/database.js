...
const getAllReservations = function (guest_id, limit = 10) {
  const query = `
    SELECT reservations.id, properties.thumbnail_photo_url, properties.title, properties.cover_photo_url, number_of_bedrooms, number_of_bathrooms, parking_spaces, properties.cost_per_night, reservations.start_date, AVG(rating) AS average_rating
    FROM reservations
...
const getAllProperties = function (options, limit = 10) {
  ...
  if (options.owner_id) {
    ...
    queryString += `${whereClause ? 'AND' : 'WHERE'} owner_id = $${queryParams.length} `;
    whereClause = true;
  }
  ...
  if (options.minimum_price_per_night && options.maximum_price_per_night) {
    ...
    queryString += `${whereClause ? 'AND' : 'WHERE'} cost_per_night >= $${queryParams.length - 1} AND cost_per_night <= $${queryParams.length} `;
    whereClause = true;
